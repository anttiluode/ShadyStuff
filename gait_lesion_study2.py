...
# =====================================================================
# 1. SPATIAL CORTEX WITH LESION CAPABILITY (UNMODIFIED)
# =====================================================================
...
# =====================================================================
# 3. LESION LAB GUI WITH SPIKE RASTER
# =====================================================================
class LesionLabApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Topological Lesion Study Lab (With Raster)")
        ...
        self.spike_history = deque(maxlen=self.RASTER_WIDTH)
    ...
    def setup_ui(self):
        ...
        # Extended tissue bar to match 4 panels
        self.tissue_canvas = tk.Canvas(grid_frame, width=1600, height=40, bg="#222", highlightthickness=0)
    ...
    def render_raster(self):
        raster_img = np.zeros((self.N_NEURONS, self.RASTER_WIDTH, 3), dtype=np.uint8)
        
        for x, frame_spikes in enumerate(self.spike_history):
            indices = np.where(frame_spikes > 0.5)[0]
            raster_img[indices, x] = [0, 255, 136] # Neon green spikes

        return cv2.resize(raster_img, (self.RES, self.RES), interpolation=cv2.INTER_NEAREST)
    ...
    def update_loop(self):
        ...
        if ret:
            ...
            # Update history for raster
            spatial_spikes = (spikes.sum(dim=(2,3)) > 0).float().squeeze().cpu().numpy()
            self.spike_history.append(spatial_spikes)
            ...
            # Panel 4: Raster
            raster_vis = self.render_raster()
            
            # Stack into 4 panels horizontally
            display = np.hstack((real_bgr, stream_vis, mind_vis, raster_vis))
            display = cv2.resize(display, (1600, 400), interpolation=cv2.INTER_NEAREST)
            ...
            cv2.putText(display, "4. SPIKE RASTER (TIME ->)", (1210, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
